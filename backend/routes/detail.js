...
router.get("/detail/:id", authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT nama, alamat, no_hp, no_whatsapp, permintaan, detail_permintaan, lokasi, surat, status, foto FROM request_data WHERE id = $1", // Use $1 as placeholder
      [id]
    );
...
router.post("/update-status/:id", authenticateToken, async (req, res) => {
  ...
  const query = "UPDATE request_data SET status = $1, date = $2 WHERE id = $3"; // Use PostgreSQL placeholders
